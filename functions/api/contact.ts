...
export const onRequestPost: PagesFunction<Env> = async ({ request, env }): Promise<Response> => {
  try {
    ...
    if (!endpoint) {
      return json("The contact form is not connected yet. Please email us directly.", 503);
    }
    if (!endpoint.startsWith("https://")) {
      return json("The contact form endpoint is not configured securely.", 500);
    }
    ...
    if (!response.ok) {
      return json("We could not send your request. Please try again or email us directly.", 502);
    }
