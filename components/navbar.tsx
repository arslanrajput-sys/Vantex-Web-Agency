...
export function Navbar() {
  ...
  return <header className={`site-header ${scrolled||open?"site-header-scrolled":""}`}><nav className="shell site-nav" aria-label="Primary navigation"><Logo/><div className="desktop-nav">{links.map((link)=><Link key={link.label} href={isHome&&link.href.startsWith("/#")?link.href.slice(1):link.href} aria-current={isLinkActive(link)?"page":undefined} className={`nav-link ${isLinkActive(link)?"active":""}`}>{link.label}</Link>)}</div><Link className="button button-sm nav-quote-button" href="/contact/#contact">Request a Free Quote <ArrowRight size={16}/></Link><button className="icon-button mobile-menu-button" onClick={()=>setOpen(!open)} aria-expanded={open} aria-controls="mobile-menu" aria-label={open?"Close navigation":"Open navigation"}>{open?<X aria-hidden="true"/>:<Menu aria-hidden="true"/>}</button></nav><div id="mobile-menu" aria-hidden={!open} inert={!open} className={`mobile-menu ${open?"mobile-menu-open":""}`}><div className="shell mobile-menu-inner">{links.map((link)=><Link key={link.label} href={isHome&&link.href.startsWith("/#")?link.href.slice(1):link.href} aria-current={isLinkActive(link)?"page":undefined} onClick={()=>setOpen(false)} className={isLinkActive(link)?"active":""}>{link.label}</Link>)}<Link className="button mobile-quote" href="/contact/#contact" onClick={()=>setOpen(false)}>Request a Free Quote <ArrowRight size={17}/></Link></div></div></header>;
}
