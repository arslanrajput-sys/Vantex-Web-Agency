...
const packages = [
  {name:"Conversion Launch",scope:"1-Page Website",price:"$199",for:"For focused offers and small businesses that need a clear, professional online presence.",features:["1 professionally designed page","Business and contact information","Contact or lead form","Mobile-responsive development","Basic on-page SEO","Speed optimization","Clear call-to-action structure"],cta:"Start My Conversion Launch"},
  {name:"Business Authority",scope:"Up to 7 Pages",price:"$499",for:"For service businesses ready to establish a complete, credible online presence.",recommended:true,features:["Up to 7 custom website pages","Service and about pages","Contact and lead forms","Mobile-responsive development","Basic on-page SEO","Analytics setup","Clear conversion paths across key pages"],cta:"Build My Business Website"},
  {name:"Growth Platform",scope:"Up to 12 Pages",price:"$899",for:"For growing businesses that need deeper content and a more capable digital platform.",features:["Up to 12 custom website pages","Structured service and content pages","Advanced forms and workflows","CMS-ready content setup","Third-party integrations","Analytics setup","Scalable content architecture"],cta:"Plan My Growth Platform"},
  {name:"Intelligent Scale",scope:"Advanced Custom Website + AI Receptionist",price:"$1,499",for:"For businesses that need a custom website with a simple, professional AI receptionist.",features:["Advanced custom website","Informative AI receptionist","Answers common customer questions","Collects lead and inquiry details","Uses business-provided information","Contact or booking handoff","Priority project support"],cta:"Request My Advanced Proposal"},
];

export function Pricing({ showHeading = true }: { showHeading?: boolean }) {
  return <section id="pricing" className={`section ${showHeading ? "" : "pricing-page-packages"}`}><div className="shell">{showHeading ? <SectionHeading align="center" label="Clear package pricing" title="A practical package for every stage of growth." copy="Choose the closest fit below. Your final quote will reflect the exact page count, content, dashboard functionality, integrations, and AI receptionist requirements."/> : <h2 className="sr-only">Website design packages</h2>}<div className={`pricing-grid ${showHeading ? "mt-12" : ""}`}>{packages.map(p => <article key={p.name} className={`pricing-card ${p.recommended ? "popular" : ""}`}><div className="pricing-badge-row">{p.recommended && <span className="popular-label">Most Popular</span>}</div><h3 className="pricing-name">{p.name}</h3><p className="pricing-scope">{p.scope}</p><div className="pricing-price"><strong>{p.price}</strong></div><p className="pricing-for">{p.for}</p><div className="pricing-divider"/><ul className="pricing-features">{p.features.map(f => <li key={f}><Check size={17}/><span>{f}</span></li>)}</ul><Link href="/contact" className={`${p.recommended ? "button" : "button-secondary"} pricing-cta`}>{p.cta}<ArrowRight size={17}/></Link></article>)}</div><p className="mx-auto mt-7 max-w-3xl text-center text-xs leading-5 text-muted">Each price covers the listed package scope. Optional pages, specialized integrations, paid services, custom data systems, or substantial copywriting are quoted separately before work begins.</p></div></section>;
}
