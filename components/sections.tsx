...
const packages = [
  ...
  {name:"Business Authority",scope:"Up to 7 Pages",price:"$499",for:"For service businesses ready to build trust with a complete, professional online presence.",recommended:true,features:["Up to 7 custom website pages","Home, about, service, and contact pages","Lead forms and inquiry routing","Mobile-first responsive development","Clear conversion paths across key pages","Foundational on-page SEO","Analytics setup"],cta:"Build My Business Website"},
  {name:"Growth Platform",scope:"Up to 12 Pages",price:"$899",for:"For growing businesses that need deeper content, smarter workflows, and room to scale.",features:["Up to 12 custom website pages","Scalable sitemap and content structure","Advanced forms and workflows","CMS-ready content setup","Third-party integrations","Analytics setup","Growth-ready technical foundation"],cta:"Plan My Growth Platform"},
  {name:"Intelligent Scale",scope:"Advanced Website + AI Receptionist",price:"Custom Quote",for:"For custom websites that pair advanced functionality with a focused, informative AI receptionist.",features:["Custom website scope and architecture","Professional AI receptionist","Answers approved business questions","Collects qualified lead details","Uses business-provided information only","Contact or booking handoff","Custom integrations quoted by scope"],cta:"Request My Custom Proposal"},
];

export function Pricing({ showHeading = true }: { showHeading?: boolean }) {
  return <section id="pricing" className={`pricing-section ${showHeading?"":"pricing-page-packages"}`}><div className="shell">{showHeading&&<div className="pricing-heading"><h2>A practical package for every stage of growth.</h2><p>Choose the closest fit below. Your final quote will reflect the exact page count, content, dashboard functionality, integrations, and AI receptionist requirements.</p></div>}{!showHeading&&<h2 className="sr-only">Website design packages</h2>}<div className="pricing-grid">{packages.map((item)=><article key={item.name} className={`pricing-card ${item.recommended?"popular":""}`}><div className="pricing-badge-row">{item.recommended&&<span className="popular-label">Most Popular</span>}</div><h3 className="pricing-name">{item.name}</h3><p className="pricing-scope">{item.scope}</p><div className={`pricing-price ${item.price==="Custom Quote"?"pricing-price-custom":""}`}><strong>{item.price}</strong></div><p className="pricing-for">{item.for}</p><div className="pricing-divider"/><ul className="pricing-features">{item.features.map((feature)=><li key={feature}><Check size={16}/><span>{feature}</span></li>)}</ul><Link href="/contact" className={`${item.recommended?"button":"button-secondary"} pricing-cta`}>{item.cta}<ArrowRight size={17}/></Link></article>)}</div><p className="pricing-disclaimer">The first three prices cover the listed package scope. Intelligent Scale is quoted after a requirements review. Optional pages, specialized integrations, paid services, custom data systems, or substantial copywriting are quoted separately before work begins.</p></div></section>;
}
