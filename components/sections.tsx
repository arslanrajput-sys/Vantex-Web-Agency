...
export function Portfolio() {
  ...
  return <section id="work" className="section"><div className="shell"><div className="portfolio-heading"><SectionHeading label="Selected work" title="Real websites, designed around the job they need to do." copy="A focused look at three recent builds across service businesses and interactive products. Every preview opens the live website."/><div className="portfolio-proof"><strong><span/>7 projects live</strong><p>Strategy, design, development, and launch—connected from the first idea to the finished experience.</p></div></div><div className="portfolio-grid portfolio-grid-home">{featuredProjects.map((project) => <PortfolioCard key={project.name} project={project}/>)}</div><div className="portfolio-home-footer"><p>Want a closer look at the range of work?</p><Link href="/portfolio" className="button-secondary">Explore the complete portfolio <ArrowRight size={17}/></Link></div></div></section>;
}
...
const packages = [
  {name:"Conversion Launch",scope:"1-Page Landing Experience",price:"$150–$200",for:"For one focused offer, service, or advertising campaign.",features:["1 conversion-focused landing page","Custom visual design","Mobile-responsive development","Contact or lead form","Basic on-page SEO","Speed optimization","Two revision rounds"],cta:"Start My Conversion Launch"},
  {name:"Business Authority",scope:"7–9 Page Business Website",price:"$350–$700",for:"For service businesses ready to establish a complete, credible online presence.",recommended:true,features:["7–9 custom website pages","Service and about pages","Contact and lead forms","Mobile-responsive development","Basic on-page SEO","Analytics setup","Three revision rounds"],cta:"Build My Business Website"},
  {name:"Growth Platform",scope:"12–15 Page Website + Dashboard",price:"$900–$1,300",for:"For growing businesses that need deeper content and a connected digital platform.",features:["12–15 custom website pages","Custom dashboard interface","Advanced forms and workflows","CMS-ready content setup","Third-party integrations","Analytics and tracking","Three revision rounds"],cta:"Plan My Growth Platform"},
  {name:"Intelligent Scale",scope:"Advanced Website + AI Chatbot",price:"$1,500–$2,500",for:"For businesses ready to scale with advanced functionality and smarter lead support.",features:["Custom multi-page website scope","AI chatbot integration","Dashboard or client portal","CRM and automation connections","Advanced interactions","Conversion tracking","Priority project support"],cta:"Request My Advanced Proposal"},
];

export function Pricing() {
  return <section id="pricing" className="section"><div className="shell"><SectionHeading align="center" label="Clear project ranges" title="A practical package for every stage of growth." copy="Choose the closest fit below. Your final quote will reflect the exact page count, content, dashboard functionality, integrations, and chatbot requirements."/><div className="pricing-grid mt-12">{packages.map(p => <article key={p.name} className={`pricing-card ${p.recommended ? "popular" : ""}`}><div className="pricing-badge-row">{p.recommended && <span className="popular-label">Recommended</span>}</div><h3 className="pricing-name">{p.name}</h3><p className="pricing-scope">{p.scope}</p><div className="pricing-price"><strong>{p.price}</strong></div><p className="pricing-for">{p.for}</p><div className="pricing-divider"/><ul className="pricing-features">{p.features.map(f => <li key={f}><Check size={17}/><span>{f}</span></li>)}</ul><Link href="/contact" className={`${p.recommended ? "button" : "button-secondary"} pricing-cta`}>{p.cta}<ArrowRight size={17}/></Link></article>)}</div><p className="mx-auto mt-7 max-w-3xl text-center text-xs leading-5 text-muted">These ranges cover the listed package scope. Specialized integrations, paid services, custom data systems, or substantial copywriting are quoted separately before work begins.</p></div></section>;
}
...
export function FinalCta() {
  return <section className="pb-24 pt-8 md:pb-32"><div className="shell"><div className="final-cta"><div><p className="section-label">Your next step</p><h2 className="max-w-3xl font-display text-3xl font-extrabold tracking-[-.045em] text-copy md:text-5xl">Ready for a website that feels unmistakably yours?</h2><p className="mt-5 max-w-2xl text-base leading-7 text-soft md:text-lg">Tell us what you need the website to achieve. We will review the project and recommend the clearest path to design, build, and launch it well.</p></div><div className="mt-8 flex flex-col gap-3 sm:flex-row"><Link href="/contact" className="button">Get My Free Website Quote <ArrowRight size={18}/></Link><a href={`tel:${site.phoneHref}`} className="button-secondary"><Phone size={17}/>{site.phoneDisplay}</a></div></div></div></section>;
}
