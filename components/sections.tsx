...
const services = [
  ...
  [Layers3,"Web development","Clean, responsive code that loads quickly, works reliably, and stays straightforward to manage."],
...
const packages = [
  {name:"Starter Landing Page",for:"For one focused offer or campaign.",features:["One premium landing page","Mobile-responsive design","Contact form","Basic SEO setup","Speed optimization","Two revision rounds"],cta:"Request a Quote"},
  {name:"Business Website",for:"For established service businesses ready to grow.",popular:true,features:["Up to five pages","Custom design","Service pages","Contact and lead forms","Mobile optimization","Basic on-page SEO","Analytics setup","Three revision rounds"],cta:"Get a Custom Proposal"},
  {name:"Growth Website",for:"For a stronger, expandable online platform.",features:["Up to ten pages","Advanced sections","Conversion-focused structure","Blog or CMS-ready setup","Custom integrations","Advanced animations","Priority support"],cta:"Request a Quote"},
];

export function Pricing() {
  return <section id="pricing" className="section"><div className="shell"><SectionHeading align="center" label="Flexible scope" title="Choose the right starting point for your business." copy="Every project receives a custom quote based on your goals, content, and technical needs. No surprise add-ons."/><div className="mt-12 grid items-stretch gap-6 lg:grid-cols-3">{packages.map(p => <article key={p.name} className={`pricing-card ${p.popular ? "popular" : ""}`}>{p.popular && <span className="popular-label">Most popular</span>}<p className="font-display text-2xl font-bold text-copy">{p.name}</p><p className="mt-3 min-h-12 text-sm leading-6 text-soft">{p.for}</p><div className="my-6 h-px bg-line"/><ul className="space-y-3">{p.features.map(f => <li key={f} className="flex gap-2.5 text-sm text-soft"><Check size={17} className="shrink-0 text-success"/>{f}</li>)}</ul><a href="#contact" className={p.popular ? "button mt-8 w-full" : "button-secondary mt-8 w-full"}>{p.cta}<ArrowRight size={17}/></a></article>)}</div></div></section>;
}
