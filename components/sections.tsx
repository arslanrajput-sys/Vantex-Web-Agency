...
export function Pricing() {
  return <section id="pricing" className="section"><div className="shell"><SectionHeading align="center" label="Clear project ranges" title="A practical package for every stage of growth." copy="Choose the closest fit below. Your final quote will reflect the exact page count, content, dashboard functionality, integrations, and chatbot requirements."/><div className="mt-12 grid items-stretch gap-6 md:grid-cols-2 xl:grid-cols-4">{packages.map(p => <article key={p.name} className={`pricing-card ${p.popular ? "popular" : ""}`}>{p.popular && <span className="popular-label">Most popular</span>}<p className="pricing-name">{p.name}</p><div className="pricing-price"><strong>{p.price}</strong><span>Typical project range</span></div><p className="pricing-for">{p.for}</p><div className="my-5 h-px bg-line"/><ul className="space-y-3">{p.features.map(f => <li key={f} className="flex gap-2.5 text-sm text-soft"><Check size={17} className="shrink-0 text-success"/>{f}</li>)}</ul><a href="#contact" className={p.popular ? "button mt-8 w-full" : "button-secondary mt-8 w-full"}>{p.cta}<ArrowRight size={17}/></a></article>)}</div><p className="mx-auto mt-7 max-w-3xl text-center text-xs leading-5 text-muted">These ranges cover the listed package scope. Specialized integrations, paid services, custom data systems, or substantial copywriting are quoted separately before work begins.</p></div></section>;
}
